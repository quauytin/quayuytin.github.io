...
return $id;
   ...
   public function check_token_api($token) {
    ...
    public function bannick_image($id, $type) {

        $path = $_SERVER["DOCUMENT_ROOT"]."/upload/bannick/".$type;
                  ...
                  if ($opendirectory = opendir($path)){
                    while (($file = readdir($opendirectory)) !== false){
                        ...
                        if ($file != "." && $file != "..") {
                            $arr = explode(".", $file);
                            if($arr[0] == $id) {
                               return '/upload/bannick/'.$type.'/'.$file;
                            }
                        }
                    }
 ...
 public function count_acc_lq($type) {
        ...
        switch ($type) {
            case 'conlai':
        $result = mysqli_query($this->connect_db(), "SELECT * FROM `nicklq` WHERE `status`='1' AND `nguoimua`='null' ");
        $row = mysqli_num_rows($result);
                break;

            case 'daban':
        $result = mysqli_query($this->connect_db(), "SELECT * FROM `nicklq` WHERE `status`='0'");
        $row = mysqli_num_rows($result);
                break;
        }
 ...
 public function count_acc_nro($type) {

        switch ($type) {
            case 'conlai':
        $result = mysqli_query($this->connect_db(), "SELECT * FROM `nicknro` WHERE `status`='1' AND `nguoimua`='null' ");
        $row = mysqli_num_rows($result);
                break;

            case 'daban':
        $result = mysqli_query($this->connect_db(), "SELECT * FROM `nicknro` WHERE `status`='0'");
        $row = mysqli_num_rows($result);
                break;
        }
 ...
 public function count_acc_pubg($type) {

        switch ($type) {
            case 'conlai':
        $result = mysqli_query($this->connect_db(), "SELECT * FROM `nickpubg` WHERE `status`='1' AND `nguoimua`='null' ");
        $row = mysqli_num_rows($result);
                break;

            case 'daban':
        $result = mysqli_query($this->connect_db(), "SELECT * FROM `nickpubg` WHERE `status`='0'");
        $row = mysqli_num_rows($result);
                break;
        }
  ...
  function get_thumb_lienquan($id) {
        $index = glob($_SERVER["DOCUMENT_ROOT"]."/upload/nicklq/thumb/".$id.".*");
        $_DOMAIN = "https://".$_SERVER["SERVER_NAME"]."/";
        if ($index) {
            $arr = explode("/", $index[0]);
            $last = array_pop($arr);
            return $_DOMAIN."upload/nicklq/thumb/".$last;
        } else {
                return $_DOMAIN."upload/nicklq/banner.jpg";
        }
        }
 
 function get_thumb_ngocrong($id) {
        $index = glob($_SERVER["DOCUMENT_ROOT"]."/upload/nicknro/thumb/".$id.".*");
        $_DOMAIN = "https://".$_SERVER["SERVER_NAME"]."/";
        if ($index) {
            $arr = explode("/", $index[0]);
            $last = array_pop($arr);
            return $_DOMAIN."upload/nicknro/thumb/".$last;
        } else {
                return $_DOMAIN."upload/nicknro/banner.jpg";
        }
        }
 


 function get_thumb_pubg($id) {
        $index = glob($_SERVER["DOCUMENT_ROOT"]."/upload/nickpubg/thumb/".$id.".*");
        $_DOMAIN = "https://".$_SERVER["SERVER_NAME"]."/";
        if ($index) {
            $arr = explode("/", $index[0]);
            $last = array_pop($arr);
            return $_DOMAIN."upload/nickpubg/thumb/".$last;
        } else {
                return $_DOMAIN."upload/nickpubg/banner.jpg";
        }
        }
